...
if [ $(/bin/ls .wrangler/state/v3/d1/miniflare-D1DatabaseObject/*.sqlite | egrep -v metadata | wc -l) != 1 ] ; then
  echo "Cannot find a single sqlite database in .wrangler."
  exit 1
fi
DB_PATH=$(/bin/ls .wrangler/state/v3/d1/miniflare-D1DatabaseObject/*.sqlite | egrep -v metadata)
...
sqlite-utils insert $DB_PATH expenseTag - < tmp/expenseTag_dump.json
