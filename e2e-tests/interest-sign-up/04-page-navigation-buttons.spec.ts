...
import { verifyOnSignInPage, verifyOnInterestSignUpPage } from '../support/page-verifiers'
...
test.describe('Interest Sign-Up Mode: Page Navigation Tests', () => {
  ...
  test('can navigate between sign-in and interest sign-up pages', async ({ page }) => {
    await navigateToSignIn(page)
  ...
  test('interest sign-up page has correct form elements', async ({ page }) => {
    ...
    expect(await page.locator('h2').textContent()).toContain('Join the Waitlist')
  })
})
