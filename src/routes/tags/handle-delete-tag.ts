 ...
 * @module routes/tags/handleDeleteTag
 */
...
import { deleteTag } from '../../lib/expense-db-access'

/**
 * Attach the delete tag handler to the app.
 */
export const handleDeleteTag = (app: Hono<{ Bindings: Bindings }>): void => {
  app.post(
    PATHS.TAGS.DELETE,
    ...
    async (c: Context) => {
      try {
        ...
        if (result.isErr) {
          console.error('Delete tag error:', result.error)
        ...
        return redirectWithMessage(c, PATHS.TAGS.LIST, EXPENSE_MESSAGES.TAG_DELETED)
      } catch (error) {
