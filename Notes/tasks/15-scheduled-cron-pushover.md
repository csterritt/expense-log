...
Read and follow the project coding standards in `Notes/skills/AGENTS.md`. Keep the file dependency-light — import only `createDbClient`, `materializeRecurring`, `todayEt`, `pushoverNotifyEnv`, and the `Bindings` type. Do NOT import Hono. Do NOT call `getCurrentTime` here — that helper depends on a request context; production cron has no cookie. Do NOT mark the file `// PRODUCTION:REMOVE` — this is the production cron entry point.
