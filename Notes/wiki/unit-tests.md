...
Catalog of all unit tests under `tests/` (9 spec files total). Each file links to its individual wiki page.
...
- [tests/expense-access.spec.ts](./tests/expense-access.spec.md) — Tests Issue 09 category repository helpers (`createCategory`, `renameCategory`, `mergeCategory`, `deleteCategory`) and Issue 10 tag repository helpers (`createTag`, `renameTag`, `mergeTag`, `deleteTag`) with an in-memory SQLite harness and atomic batch shim.
- [tests/expense-validators.spec.ts](./tests/expense-validators.spec.md) — Tests `parseExpenseCreate`, `parseNewCategoryName`, `parseTagCsv`, Issue 09 category-management validators, and Issue 10 tag-management validators (src/lib/expense-validators.ts). Covers per-field pass/fail cases, normalization, merge/delete malformed payloads, and multi-field failure aggregation.
