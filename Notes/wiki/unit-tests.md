...
- [tests/expense-validators.spec.ts](./tests/expense-validators.spec.md) — Tests `parseExpenseCreate` (src/lib/expense-validators.ts). Covers per-field pass/fail cases for description, amount, date, and category, plus multi-field failure aggregation.
