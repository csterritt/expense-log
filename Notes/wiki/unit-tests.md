...
Catalog of all unit tests under `tests/` (12 spec files total; Issue 13 adds new test cases to two existing files and a new `recurrence.spec.ts`; Issue 14 expands `recurrence.spec.ts` and `expense-access.spec.ts`; Issue 15 adds `po-notify.spec.ts` and `scheduled.spec.ts`; 2026-05-22 removes `summarize`, `parseSummaryQuery`, and `monthKeyEt`/`yearKeyEt` tests). Each file links to its individual wiki page.
