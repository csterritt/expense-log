...
- [tests/expense-validators.spec.ts](./tests/expense-validators.spec.md) — Tests `parseExpenseCreate` and `parseNewCategoryName` (src/lib/expense-validators.ts). Covers per-field pass/fail cases for description, amount, date, and category, the Issue 05 new-category-name length/empty/whitespace/case-preservation cases, plus multi-field failure aggregation.
