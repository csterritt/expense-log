...
## [2026-04-27] ingest | Progressive enhancement: category combobox + tag chip picker (JS-on) + no-JS fallback

Ingested the progressive-enhancement slice (Tasks 1–11) that adds client-side vanilla-JS modules for the expense entry form, plus new DB helpers, unit tests, E2E coverage, and a no-JS smoke test.

- **`src/lib/db/expense-access.ts`**: added `listTags(db)` helper (mirrors `listCategories` structure: `withRetry` wrapper, `asc(tag.name)` sort, `Result<TagRow[], Error>`). `Notes/wiki/src/lib/db/expense-access.md` updated with `TagRow` and `listTags` entries.
- **`tests/expense-access.spec.ts`** (new, Task 2): unit test verifying `listTags` is exported and returns a `Promise<Result<TagRow[], Error>>`. Full query/sorting/retry behaviour is covered by the E2E suite. New wiki page `tests/expense-access.spec.md`; unit-test catalog 8 → 9.
- **`src/routes/expenses/build-expenses.tsx`**: GET now fetches `listCategories` + `listTags` in parallel (re-adding the `listCategories` call removed in Issue 05) and embeds both lists as inline `<script type="application/json">` tags with stable `data-testid`s (`embedded-categories-json`, `embedded-tags-json`). The category input gains `data-category-combobox` and the tags input gains `data-tag-chip-picker`; two deferred `<script src="/js/...">` tags are appended at the end of the rendered tree so the DOM is fully parsed before the modules run. `Notes/wiki/src/routes/expenses/build-expenses.md` rewritten with the progressive-enhancement section.
- **`public/js/category-combobox.js`** (new, Tasks 5–6): vanilla-JS module auto-initialised on `DOMContentLoaded` by `[data-category-combobox]`. Reads the embedded categories JSON, renders a `role="listbox"` dropdown below the input, filters by case-insensitive substring match, supports ArrowDown/ArrowUp/Enter/Escape keyboard navigation, shows a "Create ..." row when no match exists, and writes the selected value back to the original input. Public `data-testid` surface: `category-combobox-dropdown`, `category-combobox-option-N`, `category-combobox-create-row`, `category-combobox-input`. New wiki page `public-js/category-combobox.md`; linked from `source-code.md`.
- **`public/js/tag-chip-picker.js`** (new, Tasks 8–9): vanilla-JS module auto-initialised on `DOMContentLoaded` by `[data-tag-chip-picker]`. Reads the embedded tags JSON, hides the original CSV input and creates a chip container (`role="list"`), renders filtered suggestions below (`role="listbox"`), adds chips via click or keyboard (Enter/comma), removes chips via click or Backspace, and syncs the combined CSV to a hidden input so form submission works identically to the no-JS path. Public `data-testid` surface: `tag-chip-picker-chips`, `tag-chip-picker-suggestions`, `tag-chip-picker-chip-N`, `tag-chip-picker-remove-N`, `tag-chip-picker-suggestion-N`, `tag-chip-picker-create-row`, `tag-chip-picker-input`. New wiki page `public-js/tag-chip-picker.md`; linked from `source-code.md`.
- **`e2e-tests/expenses/06-category-combobox-js.spec.ts`** (new, Task 7): 3 Playwright tests — filters and selects an existing category by click, selects by keyboard (ArrowDown+Enter), and creates a new category via the unmatched-name "Create" row. `Notes/wiki/e2e-tests/expenses/06-category-combobox-js.spec.md` added.
- **`e2e-tests/expenses/07-tag-chip-picker-js.spec.ts`** (new, Task 10): 3 Playwright tests — adds chips by click and by keyboard (comma), removes chips by click and by Backspace, and submits the form successfully with chips present. `Notes/wiki/e2e-tests/expenses/07-tag-chip-picker-js.spec.md` added.
- **`e2e-tests/expenses/08-no-js-fallback.spec.ts`** (new, Task 11): 1 Playwright test with `test.use({ javaScriptEnabled: false })` — asserts the category and tags inputs remain plain `type='text'` without combobox/chip-picker roles, fills the form with plain text values, submits successfully, and verifies the resulting expense row renders with the server-parsed tags. `Notes/wiki/e2e-tests/expenses/08-no-js-fallback.spec.md` added.
- **Catalogs**: `e2e-tests.md` count `55 → 58`, added the three new spec entries and updated the support-file description. `unit-tests.md` count `8 → 9`, added `expense-access.spec`. `source-code.md` added the new `public/js/` section with cross-links to the two module wiki pages. `index.md` added a "Progressive enhancement" section linking to the new overview page.
- **New overview page**: `Notes/wiki/progressive-enhancement.md` documents the embedded JSON contract, the `data-*` auto-init hooks, the `public/js/` module responsibilities, the public `data-testid` surface, and the progressive-enhancement guarantee (server-side behaviour is identical with or without JS).

Verification: `npx tsc --noEmit` clean on `src/` (only pre-existing `tests/send-email.spec.ts` errors remain). `bun test tests/expense-access.spec.ts` passes. `npx playwright test e2e-tests/expenses/` 24/24 pass (21 prior + 3 new).

## [2026-04-27] walkthrough | Code and UI walkthroughs for progressive enhancement

Generated code walkthrough (`Notes/walkthroughs/07-progressive-enhancement-js/walkthrough.md`) and UI walkthrough (`Notes/walkthroughs/07-progressive-enhancement-js/walkthrough-ui.md`) covering the `listTags` helper, embedded JSON contract, `data-*` auto-init hooks, the two `public/js/` modules, and the progressive-enhancement guarantee. Cross-references to all new wiki pages and E2E specs included.
