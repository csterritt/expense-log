...
- `renderConfirmCreateNew({ newCategoryName, finalCategoryName, newTagNames, finalTagNames, values })` — pure view helper for the Issue 06 generalised confirmation page. Renders a `<ul>` listing every new name the submission would create (`Create category 'foo'` first, then `Create tag 'bar'` lines alphabetised), a `<dl>` mirroring the five fields (final category + alphabetised final tag list), and a single `<form>` with hidden inputs plus Confirm/Cancel submit buttons sharing `name='action'`. Replaces the Issue 05 `renderConfirmCreateCategory` view.
