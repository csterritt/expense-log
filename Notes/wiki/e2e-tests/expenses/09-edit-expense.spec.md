...
1. **Edit page pre-populates every field** — seeds an expense with two tags (`groceries`, `dairy`), clicks `expense-row-edit`, asserts the URL matches `/expenses/:id/edit` and every form field's `value` matches the seeded row (description, amount as plain decimal via `formatCentsPlain`, today's ET date, category name). Asserts the corresponding tag chip checkboxes are pre-checked. Asserts `expense-form-save` button is visible.
