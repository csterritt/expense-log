...
- [e2e-tests/recurring/09-create-tag-chip-ui.spec.ts](./e2e-tests/recurring/09-create-tag-chip-ui.spec.md) — Tag chip-checkbox UI on the recurring create form: seeded tags render as alphabetically-ordered chips; toggling chips updates native checkboxes; selected chips are visually distinct; submitted form attaches selected tags to the created recurring template; new-tag input reaches `confirm-recurring-create-new-page`; cancel preserves chip selections and `newTags` text.
- [e2e-tests/recurring/10-edit-tag-chip-ui.spec.ts](./e2e-tests/recurring/10-edit-tag-chip-ui.spec.md) — Tag chip-checkbox UI on the recurring edit form: pre-existing tag attachments render as selected chips; toggling a chip off detaches the tag on save; toggling a chip on adds the tag on save; new-tag input reaches `confirm-recurring-edit-new-page`; cancel preserves chip selections and `newTags` text.
